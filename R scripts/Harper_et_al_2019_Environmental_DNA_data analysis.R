#' ---
#' Title: "Validation of environmental DNA metabarcoding for ecological hypothesis testing: a case study on a threatened amphibian"
...
#' We aim to test biotic and abiotic determinants of GCN occupancy, and the 
#' status of the GCN as a flagship pond species by looking at whether 
#' vertebrate species richness is associated with GCN presence-absence and
#' the GCN Habitat Suitability Index.
#' 
#' 
...
#' 
#'
#' Examine the effect of vertebrate group species richness on GCN occupancy.
#' 
...
richnessGCN <- glmer(Triturus_cristatus ~ (1|Sample) + Fish + Amphibian 
...
overdisp.glmer(richnessGCN)

## Overdispersion test (chi-square)
1-pchisq(586.19, df=525)    # model overdispersed
...
plot(sresid ~ vert.binary$Fish)
plot(sresid ~ vert.binary$Amphibian)
plot(sresid ~ vert.binary$Waterfowl)
plot(sresid ~ vert.binary$Terrestrial.bird)
plot(sresid ~ vert.binary$Mammal)
...
hoslem.test(vert.binary$Triturus_cristatus, fitted(richnessGCN))

## Model does not fit well as p-value is highly significant 
...
#'
#' A tree of size of 17 is optimal i.e. 17 explanatory variables. 
...
#' habitat will greatly influence pond occupancy e.g. shading and grassland.
#' 
...
#'   ruderals, woodland)
#'   
#' I will apply step-wise down nested model selection to each group using glmer.
#' Use drop1() function to select order that variables should be dropped 
#' from the model. Function computes all the single terms 
...
## modelled
GCNdist <- data.frame(habitat.dat[,c(1,10,14:15,17:18,20:24,26:29,31:32,
...
biotic.m0 <- glmer(Triturus_cristatus ~ (1|Sample) 
...
biotic.m1 <- glmer(Triturus_cristatus ~ (1|Sample)
...
biotic.m2 <- glmer(Triturus_cristatus ~ (1|Sample)
                   + Lissotriton_vulgaris + Bufo_bufo
                   + Fish + Waterfowl + Cyprinus_carpio 
                   + Gasterosteus_aculeatus + Pungitius_pungitius 
                   + Gallinula_chloropus,
...
biotic.m3 <- glmer(Triturus_cristatus ~ (1|Sample)
                   + Lissotriton_vulgaris + Bufo_bufo
...
## AIC value
biotic.m4 <- glmer(Triturus_cristatus ~ (1|Sample)
...
biotic.m5 <- glmer(Triturus_cristatus ~ (1|Sample)
...
## p-value from LRT not significant and reduction in AIC value
## so removal of waterfowl presence has reduced model fit
...
biotic.m6 <- glmer(Triturus_cristatus ~ (1|Sample)
                   + Lissotriton_vulgaris + Bufo_bufo + Waterfowl 
...
## p-value from LRT not significant and reduction in AIC value
## so removal of three-spined stickleback presence has reduced model fit
...
# Final biotic model: 
biotic <- glmer(Triturus_cristatus ~ (1|Sample)
                + Lissotriton_vulgaris + Bufo_bufo + Waterfowl 
                + Cyprinus_carpio + Gasterosteus_aculeatus
                + Gallinula_chloropus,
                family = binomial(link="logit"),
...
## marginal R-squared = 27.8% (proportion of variance explained by fixed effects)
## conditional R-squared = 27.8% (proportion of variance explained by fixed + random effects)

## Check for model overdispersion 
## Obtain residual deviance and degrees of freedom
overdisp.glmer(biotic)

## Overdispersion test (chi-square)
1-pchisq(497.396, df=495)   # final model not overdispersed
...
abiotic.m0 <- glmer(Triturus_cristatus ~ (1|Sample) 
...
abiotic.m1 <- glmer(Triturus_cristatus ~ (1|Sample) 
...
abiotic.m2 <- glmer(Triturus_cristatus ~ (1|Sample) 
                    + Max_Depth + Pond_Density + Inflow + Pond_Area  
                    + Outflow + Macrophytes + Quality + Permanance
...
abiotic.m3 <- glmer(Triturus_cristatus ~ (1|Sample) 
                    + Max_Depth + Pond_Density + Inflow + Pond_Area  
                    + Macrophytes + Quality + Permanance + Shading 
                    + Ruderals + Woodland,
                    family = binomial(link="logit"),
                    data = GCNdist)
...
abiotic.m4 <- glmer(Triturus_cristatus ~ (1|Sample) 
                    + Max_Depth + Pond_Density + Inflow + Pond_Area  
                    + Macrophytes + Quality + Shading + Ruderals 
                    + Woodland,
                    family = binomial(link="logit"),
                    data = GCNdist)
...
abiotic.m5 <- glmer(Triturus_cristatus ~ (1|Sample) 
                    + Max_Depth + Pond_Density + Inflow + Pond_Area  
                    + Macrophytes + Quality + Shading + Ruderals,
                    family = binomial(link="logit"),
                    data = GCNdist)
...
abiotic.m6 <- glmer(Triturus_cristatus ~ (1|Sample) 
                    + Max_Depth + Pond_Density + Inflow + Pond_Area  
...
abiotic.m7 <- glmer(Triturus_cristatus ~ (1|Sample) 
                    + Max_Depth + Pond_Density + Inflow + Pond_Area  
                    + Shading + Ruderals,
                    family = binomial(link="logit"),
                    data = GCNdist)
...
abiotic.m8 <- glmer(Triturus_cristatus ~ (1|Sample) 
                    + Max_Depth + Inflow + Pond_Area + Shading 
                    + Ruderals,
                    family = binomial(link="logit"),
                    data = GCNdist)
...
abiotic.m9 <- glmer(Triturus_cristatus ~ (1|Sample) 
                    + Inflow + Pond_Area + Shading + Ruderals,
                    family = binomial(link="logit"),
                    data = GCNdist)
...
abiotic.m10 <- glmer(Triturus_cristatus ~ (1|Sample) 
...
abiotic.m11 <- glmer(Triturus_cristatus ~ (1|Sample) 
                     + Inflow + Shading,
                     family = binomial(link="logit"),
                     data = GCNdist)
...
## Remove shading from model as this has the next lowest AIC value
## after pond area to see if final model has been reached
abiotic.m12 <- glmer(Triturus_cristatus ~ (1|Sample) 
                     + Inflow + Pond_Area,
                     family = binomial(link="logit"),
                     data = GCNdist)
...
## p-value from LRT significant and increase in AIC value (>2)
## so removal of shading has reduced model fit.


## Final abiotic model:
abiotic <- glmer(Triturus_cristatus ~ (1|Sample)
                 + Inflow + Pond_Area + Shading,
                 family = binomial(link="logit"),
                 data = GCNdist)
...
## marginal R-squared = 9.73% (proportion of variance explained by fixed effects)
## conditional R-squared = 9.85% (proportion of variance explained by fixed + random effects)

## Check for model overdispersion 
## Obtain residual deviance and degrees of freedom
overdisp.glmer(abiotic)

## Overdispersion test (chi-square)
1-pchisq(564.438, df=499)   # overdispersed
...
#' ---
#' 
#' Now, test HSI score for correlation with GCN presence-absence.
#' 
...
## Calculate R-squared of model
r.squaredGLMM(HSIm)
## marginal R-squared = 5.45% (proportion of variance explained by fixed effects)
## conditional R-squared = 5.45% (proportion of variance explained by fixed + random effects)
...
## Overdispersion test (chi-square)
1-pchisq(578.53, df=501)   # overdispersed
...
#' ---
#' 
#' Does the GCN deserve its status as a flagship pond species?
#' 
...
#' presence and the GCN habitat suitability index score (i.e. habitat 
#' suitable for GCN is suitable for other species).
#' 
#' Model vertebrate species richness against GCN presence-absence and HSI
#' score in a GLMM with Poisson distribution.
#' 

SRm <- glmer(Sp.richness ~ (1|Sample) + Triturus_cristatus + HSI,
             family=poisson,
             data=habitat.dat)

## Compare to null model
SRnull <- glmer(Sp.richness ~ (1|Sample) + 1,
                family=poisson,
                data=habitat.dat)
...
## Calculate R-squared of model
r.squaredGLMM(SRm)
## marginal R-squared = 8.02% (proportion of variance explained by fixed effects)
## conditional R-squared = 13.63% (proportion of variance explained by fixed + random effects)
...
## Overdispersion test (chi-square)
1-pchisq(557.344, df=500)   # overdispersed
...
qqnorm(sresid, cex=1.8, pch=20)
qqline(sresid, lty=2, lwd=2)
shapiro.test(sresid) # P = 9.024e-12
